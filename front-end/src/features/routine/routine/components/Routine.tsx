import { TaskGroup } from '@/features/routine/group/components/Group';
import type { Routine } from '@/features/routine/routine/types/routine.domain.type';
import { AddGroupButton } from '../../group/components/AddGroupButton';
  ...
  return (
    <main className="w-full min-h-fit sm:max-h-135 flex flex-col gap-4 scrollbar-none rounded-xl border-surface2">
      {routine?.groups.map((group) => (
        <TaskGroup key={`${group.id}-${routine.id}`} group={group} />
      ))}
