import { Task } from '@/features/routine/task/components/Task';
import type { TaskGroup } from '@/features/routine/group/types/group.domain.type';
import { useEditMode } from '@/context/EditModeContext';
import { ExpandButton } from '@/shared/components/ui/ExpandButton';
import { useExpand } from '@/shared/hooks/useExpand';
import { cn } from '@/shared/util';
import { GroupControls } from '@/features/routine/group/components/GroupControls';
import { AddTaskButton } from '../../task/components/AddTaskButton';
    ...
    <div className="text-left text-muted text-xs flex flex-col gap-3 text-3 border-2 border-b-6 pt-2 px-3 bg-bg rounded-xl border-surface2">
      <div className="flex gap-2 items-center justify-between w-full mt-2 pb-2">
        ...
        {group.tasks?.length > 0 && !isEditMode && (
          <ExpandButton onClick={controlExpand} isExpanded={isExpanded} />
        )}
      </div>
      ...
      <div
        className={cn(
          'grid transition-all duration-500 ease-out w-full',
          isEditMode || group.tasks.length === 0
            ? 'grid-rows-[1fr] opacity-100'
            : 'grid-rows-[0fr] opacity-0',
        )}
      >
